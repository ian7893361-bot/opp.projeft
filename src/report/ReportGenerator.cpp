...
std::string ReportGenerator::reportPath(const std::string& studentId,
    ...
    return "reports/" + yearMonth + "/report_" + studentId + ".txt";
}
...
bool ReportGenerator::generate(const Student* student,
    ...
    // 寫報表
    ConsoleUtils::createDirectory("reports/" + yearMonth);
    ...
    // 分類統計
    fout << "【分類統計】\n" << line80d << "\n";
    for (const auto& pair : catExpense) {
        fout << std::left << std::setw(16) << pair.first
